...
def main():
    ...
    handler = FTPHandler
    users.add_anonymous("../app", perm="el")
    results = get_teacher_msgs()
    if results:
        for row in results:
            users.add_user(row[0], row[1], f"../app/{row[2]}", perm="elrwdmf")
            users.override_perm('anonymous', f"../app/{row[2]}/上传",
                                perm="elw", recursive=True)
            users.override_perm('anonymous', f"../app/{row[2]}/下载",
